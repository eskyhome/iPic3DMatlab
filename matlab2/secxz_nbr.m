...
jsxb=squeeze(sum(Jxs1(1:end,1:end,it-aver:it),3))/(aver+1);
jsyb=squeeze(sum(Jys1(1:end,1:end,it-aver:it),3))/(aver+1);
jszb=squeeze(sum(Jzs1(1:end,1:end,it-aver:it),3))/(aver+1);
rrho=squeeze(sum(rhos0(1:end,1:end,it-aver:it),3))/(aver+1);
rrhob=squeeze(sum(rhos1(1:end,1:end,it-aver:it),3))/(aver+1);
...
strf=vecpot(xx,yy,(jsx0)./(rrhob+rrho),(jsy0)./(rrhob+rrho));
...
coplot(xx,yy,(rrho)*4*pi,ay,'x/d_i','y/d_i',['\rho_{e}(\omega_{ci}t=' num2str(time(it)) ')'])
...
coplot(xx,yy,rrhob,ay,'x/d_i','y/d_i',['\rho_i(\omega_{ci}t=' num2str(time(it)) ')'])
...
h=figure(6);
set(h,'Position' , [1125 550 560 420]);
coplot(xx,yy,(jsxb)./(rrhob)/va,ay,'x/d_i','y/d_i',['V_{ix}/v_a(\omega_{ci}t=' num2str(time(it)) ')'])
