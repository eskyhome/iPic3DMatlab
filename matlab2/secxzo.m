...
skippo=6    
bbx=squeeze(sum(Bx(1+skippo:end-skippo,1+skippo:end-skippo,it-aver:it),3))/(aver+1);
bby=squeeze(sum(By(1+skippo:end-skippo,1+skippo:end-skippo,it-aver:it),3))/(aver+1);
bbz=squeeze(sum(Bz(1+skippo:end-skippo,1+skippo:end-skippo,it-aver:it),3))/(aver+1);
eex=squeeze(sum(Ex(1+skippo:end-skippo,1+skippo:end-skippo,it-aver:it),3))/(aver+1);
eey=squeeze(sum(Ey(1+skippo:end-skippo,1+skippo:end-skippo,it-aver:it),3))/(aver+1);
eez=squeeze(sum(Ez(1+skippo:end-skippo,1+skippo:end-skippo,it-aver:it),3))/(aver+1);
jsx0=squeeze(sum(Jxs0(1+skippo:end-skippo,1+skippo:end-skippo,it-aver:it),3))/(aver+1);
jsy0=squeeze(sum(Jys0(1+skippo:end-skippo,1+skippo:end-skippo,it-aver:it),3))/(aver+1);
jsz0=squeeze(sum(Jzs0(1+skippo:end-skippo,1+skippo:end-skippo,it-aver:it),3))/(aver+1);
jsxb=squeeze(sum(Jxs2(1+skippo:end-skippo,1+skippo:end-skippo,it-aver:it),3))/(aver+1);
jsyb=squeeze(sum(Jys2(1+skippo:end-skippo,1+skippo:end-skippo,it-aver:it),3))/(aver+1);
jszb=squeeze(sum(Jzs2(1+skippo:end-skippo,1+skippo:end-skippo,it-aver:it),3))/(aver+1);
rrho=squeeze(sum(rhos0(1+skippo:end-skippo,1+skippo:end-skippo,it-aver:it),3))/(aver+1);
rrhob=squeeze(sum(rhos2(1+skippo:end-skippo,1+skippo:end-skippo,it-aver:it),3))/(aver+1);
...
[xx yy]=meshgrid(1+skippo:ny-skippo,1+skippo:nx-skippo);
