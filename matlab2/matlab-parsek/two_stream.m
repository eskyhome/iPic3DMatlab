...
aviobj = avifile ( 'phase_space.avi', 'fps', num_frames_per_second ); 
...
vXEdge = linspace(-80E-8,80E-8,128);
...
%   mHist2d = hist2d([x0(:,i), p(:,1)],vYEdge,vXEdge);
  vector1 = find(p>0);
  vector2 = find(p<0);
  plot(x0(vector1,i),p(vector1,1),'k.','MarkerSize',.2);
  hold on
  plot(x0(vector2,i),p(vector2,1),'r.','MarkerSize',.2);
  hold off;
 % hold on;
%   % calculate the classical momentum
   %p = (q1(:,i).*u1(:,i));
