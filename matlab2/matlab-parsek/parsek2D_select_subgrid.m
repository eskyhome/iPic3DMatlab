...
%   Modified by: Giovanni Lapenta for new versions of MATLAB
...
global results_dir variable_list cycle_list
...
global x_target y_target x1_sub x2_sub y1_sub y2_sub

results_dir='/TB1/newman/Lapenta/run86/1836Ccont/' ; % directory for results
...
ix_target=floor(x_target/Lxc)+1;
iy_target=floor(y_target/Lyc)+1;
ix_min=max(1,ix_target-1);
iy_min=max(1,iy_target-1);
ix_min=min(XLEN-2,ix_min);
iy_min=min(YLEN-2,iy_min);

x1_sub=Lxc*double(ix_min-1);
x2_sub=x1_sub+3*Lxc;
y1_sub=Lyc*double(iy_min-1);
y2_sub=y1_sub+3*Lyc;

% redefine Nprocs=9 for 3x3 subset of processors with XLEN=3 and YLEN=3
...
for i=1:3
    for j=1:3
        MAP(i,j)=(i-1)*3+j;
        jproc(MAP(i,j))=(ix_min+i-2)*YLEN_orig+(iy_min+j-2);
    end
end
...
info=hdf5info([processor_name,num2str(jproc(PROC)),'.hdf']);
...
%%
       if strcmp(info.GroupHierarchy.Groups(i).Groups(ii).Name,'/fields/Bx') & strfind (variable_list, 'B')
            ...
            time_keep=0;
            for time=1:ntime
              Bx_time_comp_str=regexprep(info.GroupHierarchy.Groups(i).Groups(ii).Datasets(time).Name,'/fields/Bx/cycle_','');
              Bx_time_comp_str=[' ',Bx_time_comp_str,' '];
              Bx_time_comp=str2double(Bx_time_comp_str);
              if strfind(cycle_list, Bx_time_comp_str)
                  time_keep=time_keep+1;
                  Bx_slab(:,:,time_keep,PROC)=hdf5read(info.GroupHierarchy.Groups(i).Groups(ii).Datasets(time));
                  Bx_time=[Bx_time;Bx_time_comp];
              end
           end
...
%%
       if strcmp(info.GroupHierarchy.Groups(i).Groups(ii).Name,'/fields/By') & strfind (variable_list, 'B')
            ...
            time_keep=0;
            for time=1:ntime
              By_time_comp_str=regexprep(info.GroupHierarchy.Groups(i).Groups(ii).Datasets(time).Name,'/fields/By/cycle_','');
              By_time_comp_str=[' ',By_time_comp_str,' '];
              By_time_comp=str2double(By_time_comp_str);
              if strfind(cycle_list, By_time_comp_str)
                  time_keep=time_keep+1;
                  By_slab(:,:,time_keep,PROC)=hdf5read(info.GroupHierarchy.Groups(i).Groups(ii).Datasets(time));
                  By_time=[By_time;By_time_comp];
              end
           end
...
%%
       if strcmp(info.GroupHierarchy.Groups(i).Groups(ii).Name,'/fields/Bz') & strfind (variable_list, 'B')
            ...
            time_keep=0;
            for time=1:ntime
              Bz_time_comp_str=regexprep(info.GroupHierarchy.Groups(i).Groups(ii).Datasets(time).Name,'/fields/Bz/cycle_','');
              Bz_time_comp_str=[' ',Bz_time_comp_str,' '];
              Bz_time_comp=str2double(Bz_time_comp_str);
              if strfind(cycle_list, Bz_time_comp_str)
                  time_keep=time_keep+1;
                  Bz_slab(:,:,time_keep,PROC)=hdf5read(info.GroupHierarchy.Groups(i).Groups(ii).Datasets(time));
                  Bz_time=[Bz_time;Bz_time_comp];
              end
           end
...
%%
       if strcmp(info.GroupHierarchy.Groups(i).Groups(ii).Name,'/fields/Ex') & strfind (variable_list, 'E')
            ...
            time_keep=0;
            for time=1:ntime
              Ex_time_comp_str=regexprep(info.GroupHierarchy.Groups(i).Groups(ii).Datasets(time).Name,'/fields/Ex/cycle_','');
              Ex_time_comp_str=[' ',Ex_time_comp_str,' '];
              Ex_time_comp=str2double(Ex_time_comp_str);
              if strfind(cycle_list, Ex_time_comp_str)
                  time_keep=time_keep+1;
                  Ex_slab(:,:,time_keep,PROC)=hdf5read(info.GroupHierarchy.Groups(i).Groups(ii).Datasets(time));
                  Ex_time=[Ex_time;Ex_time_comp];
              end
           end
...
%%
       if strcmp(info.GroupHierarchy.Groups(i).Groups(ii).Name,'/fields/Ey') & strfind (variable_list, 'E')
            ...
            time_keep=0;
            for time=1:ntime
              Ey_time_comp_str=regexprep(info.GroupHierarchy.Groups(i).Groups(ii).Datasets(time).Name,'/fields/Ey/cycle_','');
              Ey_time_comp_str=[' ',Ey_time_comp_str,' '];
              Ey_time_comp=str2double(Ey_time_comp_str);
              if strfind(cycle_list, Ey_time_comp_str)
                  time_keep=time_keep+1;
                  Ey_slab(:,:,time_keep,PROC)=hdf5read(info.GroupHierarchy.Groups(i).Groups(ii).Datasets(time));
                  Ey_time=[Ey_time;Ey_time_comp];
              end
           end
...
%%
       if strcmp(info.GroupHierarchy.Groups(i).Groups(ii).Name,'/fields/Ez') & strfind (variable_list, 'E')
            ...
            time_keep=0;
            for time=1:ntime
              Ez_time_comp_str=regexprep(info.GroupHierarchy.Groups(i).Groups(ii).Datasets(time).Name,'/fields/Ez/cycle_','');
              Ez_time_comp_str=[' ',Ez_time_comp_str,' '];
              Ez_time_comp=str2double(Ez_time_comp_str);
              if strfind(cycle_list, Ez_time_comp_str)
                  time_keep=time_keep+1;
                  Ez_slab(:,:,time_keep,PROC)=hdf5read(info.GroupHierarchy.Groups(i).Groups(ii).Datasets(time));
                  Ez_time=[Ez_time;Ez_time_comp];
              end
           end
...
%%
       if strcmp(info.GroupHierarchy.Groups(i).Groups(ii).Name,'/potentials/Ax') & strfind (variable_list, 'A')
            ...
            time_keep=0;
            for time=1:ntime
              Ax_time_comp_str=regexprep(info.GroupHierarchy.Groups(i).Groups(ii).Datasets(time).Name,'/potentials/Ax/cycle_','');
              Ax_time_comp_str=[' ',Ax_time_comp_str,' '];
              Ax_time_comp=str2double(Ax_time_comp_str);
              if strfind(cycle_list, Ax_time_comp_str)
                  time_keep=time_keep+1;
                  Ax_slab(:,:,time_keep,PROC)=hdf5read(info.GroupHierarchy.Groups(i).Groups(ii).Datasets(time));
                  Ax_time=[Ax_time;Ax_time_comp];
              end
           end
...
%%
       if strcmp(info.GroupHierarchy.Groups(i).Groups(ii).Name,'/potentials/Ay') & strfind (variable_list, 'A')
            ...
            time_keep=0;
            for time=1:ntime
              Ay_time_comp_str=regexprep(info.GroupHierarchy.Groups(i).Groups(ii).Datasets(time).Name,'/potentials/Ay/cycle_','');
              Ay_time_comp_str=[' ',Ay_time_comp_str,' '];
              Ay_time_comp=str2double(Ay_time_comp_str);
              if strfind(cycle_list, Ay_time_comp_str)
                  time_keep=time_keep+1;
                  Ay_slab(:,:,time_keep,PROC)=hdf5read(info.GroupHierarchy.Groups(i).Groups(ii).Datasets(time));
                  Ay_time=[Ay_time;Ay_time_comp];
              end
           end
...
%%
       if strcmp(info.GroupHierarchy.Groups(i).Groups(ii).Name,'/potentials/Az') & strfind (variable_list, 'A')
            ...
            time_keep=0;
            for time=1:ntime
              Az_time_comp_str=regexprep(info.GroupHierarchy.Groups(i).Groups(ii).Datasets(time).Name,'/potentials/Az/cycle_','');
              Az_time_comp_str=[' ',Az_time_comp_str,' '];
              Az_time_comp=str2double(Az_time_comp_str);
              if strfind(cycle_list, Az_time_comp_str)
                  time_keep=time_keep+1;
                  Az_slab(:,:,time_keep,PROC)=hdf5read(info.GroupHierarchy.Groups(i).Groups(ii).Datasets(time));
                  Az_time=[Az_time;Az_time_comp];
              end
           end
...
%%
       if strcmp(info.GroupHierarchy.Groups(i).Groups(ii).Name,'/potentials/phi') & strfind (variable_list, 'phi')
            ...
            time_keep=0;
            for time=1:ntime
              phi_time_comp_str=regexprep(info.GroupHierarchy.Groups(i).Groups(ii).Datasets(time).Name,'/potentials/phi/cycle_','');
              phi_time_comp_str=[' ',phi_time_comp_str,' '];
              phi_time_comp=str2double(phi_time_comp_str);
              if strfind(cycle_list, phi_time_comp_str)
                  time_keep=time_keep+1;
                  phi_slab(:,:,time_keep,PROC)=hdf5read(info.GroupHierarchy.Groups(i).Groups(ii).Datasets(time));
                  phi_time=[phi_time;phi_time_comp];
              end
           end
...
%%                 
       if strcmp(info.GroupHierarchy.Groups(i).Groups(ii).Groups(iii).Name,['/moments/species_',num2str(nspec-1),'/Jx']) & strfind (variable_list, 'J')
           ...
           time_keep=0;
           for time=1:ntime
               Jxs_time_comp_str=regexprep(info.GroupHierarchy.Groups(i).Groups(ii).Groups(iii).Datasets(time).Name,['/moments/species_',num2str(nspec-1),'/Jx','/cycle_'], '');
               Jxs_time_comp_str=[' ',Jxs_time_comp_str,' '];
               Jxs_time_comp=str2double(Jxs_time_comp_str);
               if strfind(cycle_list,Jxs_time_comp_str)
                   time_keep=time_keep+1;
                   eval(['Jxs_slab' num2str(nspec-1) '(:,:,time_keep,PROC) = hdf5read(info.GroupHierarchy.Groups(i).Groups(ii).Groups(iii).Datasets(time));']);
                   Jxs_time=[Jxs_time;Jxs_time_comp];
               end
           end
...
%%                 
       if strcmp(info.GroupHierarchy.Groups(i).Groups(ii).Groups(iii).Name,['/moments/species_',num2str(nspec-1),'/Jy']) & strfind (variable_list, 'J')
           ...
           time_keep=0;
           for time=1:ntime
               Jys_time_comp_str=regexprep(info.GroupHierarchy.Groups(i).Groups(ii).Groups(iii).Datasets(time).Name,['/moments/species_',num2str(nspec-1),'/Jy','/cycle_'], '');
               Jys_time_comp_str=[' ',Jys_time_comp_str,' '];
               Jys_time_comp=str2double(Jys_time_comp_str);
               if strfind(cycle_list,Jys_time_comp_str)
                   time_keep=time_keep+1;
                   eval(['Jys_slab' num2str(nspec-1) '(:,:,time_keep,PROC) = hdf5read(info.GroupHierarchy.Groups(i).Groups(ii).Groups(iii).Datasets(time));']);
                   Jys_time=[Jys_time;Jys_time_comp];
               end
           end
...
%%                 
       if strcmp(info.GroupHierarchy.Groups(i).Groups(ii).Groups(iii).Name,['/moments/species_',num2str(nspec-1),'/Jz']) & strfind (variable_list, 'J')
           ...
           time_keep=0;
           for time=1:ntime
               Jzs_time_comp_str=regexprep(info.GroupHierarchy.Groups(i).Groups(ii).Groups(iii).Datasets(time).Name,['/moments/species_',num2str(nspec-1),'/Jz','/cycle_'], '');
               Jzs_time_comp_str=[' ',Jzs_time_comp_str,' '];
               Jzs_time_comp=str2double(Jzs_time_comp_str);
               if strfind(cycle_list,Jzs_time_comp_str)
                   time_keep=time_keep+1;
                   eval(['Jzs_slab' num2str(nspec-1) '(:,:,time_keep,PROC) = hdf5read(info.GroupHierarchy.Groups(i).Groups(ii).Groups(iii).Datasets(time));']);
                   Jzs_time=[Jzs_time;Jzs_time_comp];
               end
           end
...
%%                 
       if strcmp(info.GroupHierarchy.Groups(i).Groups(ii).Groups(iii).Name,['/moments/species_',num2str(nspec-1),'/rho']) & strfind (variable_list, 'rho')
           ...
           time_keep=0;
           for time=1:ntime
               rhos_time_comp_str=regexprep(info.GroupHierarchy.Groups(i).Groups(ii).Groups(iii).Datasets(time).Name,['/moments/species_',num2str(nspec-1),'/rho','/cycle_'], '');
               rhos_time_comp_str=[' ',rhos_time_comp_str,' '];
               rhos_time_comp=str2double(rhos_time_comp_str);
               if strfind(cycle_list,rhos_time_comp_str)
                   time_keep=time_keep+1;
                   eval(['rhos_slab' num2str(nspec-1) '(:,:,time_keep,PROC) = hdf5read(info.GroupHierarchy.Groups(i).Groups(ii).Groups(iii).Datasets(time));']);
                   rhos_time=[rhos_time;rhos_time_comp];
               end
           end
...
%%                 
                          ...
                          if strcmp(info.GroupHierarchy.Groups(i).Groups(ii).Groups(iii).Name,['/particles/species_',num2str(nspec-1),'/x']) & strfind (variable_list, 'x') 
                                ...
                                time_keep=0;
                                for time=1:ntime
                                    x_time_comp_str=regexprep(info.GroupHierarchy.Groups(i).Groups(ii).Groups(iii).Datasets(time).Name,['/particles/species_',num2str(nspec-1),'/x','/cycle_'], '');
                                    x_time_comp_str=[' ',x_time_comp_str,' '];
                                    x_time_comp=str2double(x_time_comp_str);
                                    if strfind (cycle_list, x_time_comp_str) 
                                      time_keep=time_keep+1;
                                      eval(['x_slab' num2str(nspec-1) '.time' num2str(time_keep) '.PROC' num2str(PROC) '= hdf5read(info.GroupHierarchy.Groups(i).Groups(ii).Groups(iii).Datasets(time));']);
                                      x_time=[x_time;x_time_comp];
                                    end
                                end
                                ntime_keep=time_keep;
...
%%                 
                          ...
                          if strcmp(info.GroupHierarchy.Groups(i).Groups(ii).Groups(iii).Name,['/particles/species_',num2str(nspec-1),'/y']) & strfind (variable_list, 'x') 
                                ...
                                time_keep=0;
                                for time=1:ntime
                                    y_time_comp_str=regexprep(info.GroupHierarchy.Groups(i).Groups(ii).Groups(iii).Datasets(time).Name,['/particles/species_',num2str(nspec-1),'/y','/cycle_'], '');
                                    y_time_comp_str=[' ',y_time_comp_str,' '];
                                    y_time_comp=str2double(y_time_comp_str);
                                    if strfind (cycle_list, y_time_comp_str) 
                                      time_keep=time_keep+1;
                                      eval(['y_slab' num2str(nspec-1) '.time' num2str(time_keep) '.PROC' num2str(PROC) '= hdf5read(info.GroupHierarchy.Groups(i).Groups(ii).Groups(iii).Datasets(time));']);
                                      y_time=[y_time;y_time_comp];
                                    end
                                end
                                ntime_keep=time_keep;
...
%%                 
                          ...
                          if strcmp(info.GroupHierarchy.Groups(i).Groups(ii).Groups(iii).Name,['/particles/species_',num2str(nspec-1),'/z']) & strfind (variable_list, 'x') 
                                ...
                                time_keep=0;
                                for time=1:ntime
                                    z_time_comp_str=regexprep(info.GroupHierarchy.Groups(i).Groups(ii).Groups(iii).Datasets(time).Name,['/particles/species_',num2str(nspec-1),'/z','/cycle_'], '');
                                    z_time_comp_str=[' ',z_time_comp_str,' '];
                                    z_time_comp=str2double(z_time_comp_str);
                                    if strfind (cycle_list, z_time_comp_str) 
                                      time_keep=time_keep+1;
                                      eval(['z_slab' num2str(nspec-1) '.time' num2str(time_keep) '.PROC' num2str(PROC) '= hdf5read(info.GroupHierarchy.Groups(i).Groups(ii).Groups(iii).Datasets(time));']);
                                      z_time=[z_time;z_time_comp];
                                    end
                                end
                                ntime_keep=time_keep;
...
%%                 
                          ...
                          if strcmp(info.GroupHierarchy.Groups(i).Groups(ii).Groups(iii).Name,['/particles/species_',num2str(nspec-1),'/u']) & strfind (variable_list, 'v') 
                                ...
                                time_keep=0;
                                for time=1:ntime
                                    u_time_comp_str=regexprep(info.GroupHierarchy.Groups(i).Groups(ii).Groups(iii).Datasets(time).Name,['/particles/species_',num2str(nspec-1),'/u','/cycle_'], '');
                                    u_time_comp_str=[' ',u_time_comp_str,' '];
                                    u_time_comp=str2double(u_time_comp_str);
                                    if strfind (cycle_list, u_time_comp_str) 
                                      time_keep=time_keep+1;
                                      eval(['u_slab' num2str(nspec-1) '.time' num2str(time_keep) '.PROC' num2str(PROC) '= hdf5read(info.GroupHierarchy.Groups(i).Groups(ii).Groups(iii).Datasets(time));']);
                                      u_time=[u_time;u_time_comp];
                                    end
                                end
                                ntime_keep=time_keep;
...
%%                 
                          ...
                          if strcmp(info.GroupHierarchy.Groups(i).Groups(ii).Groups(iii).Name,['/particles/species_',num2str(nspec-1),'/v']) & strfind (variable_list, 'v') 
                                ...
                                time_keep=0;
                                for time=1:ntime
                                    v_time_comp_str=regexprep(info.GroupHierarchy.Groups(i).Groups(ii).Groups(iii).Datasets(time).Name,['/particles/species_',num2str(nspec-1),'/v','/cycle_'], '');
                                    v_time_comp_str=[' ',v_time_comp_str,' '];
                                    v_time_comp=str2double(v_time_comp_str);
                                    if strfind (cycle_list, v_time_comp_str) 
                                      time_keep=time_keep+1;
                                      eval(['v_slab' num2str(nspec-1) '.time' num2str(time_keep) '.PROC' num2str(PROC) '= hdf5read(info.GroupHierarchy.Groups(i).Groups(ii).Groups(iii).Datasets(time));']);
                                      v_time=[v_time;v_time_comp];
                                    end
                                end
                                ntime_keep=time_keep;
...
%%                 
                          ...
                          if strcmp(info.GroupHierarchy.Groups(i).Groups(ii).Groups(iii).Name,['/particles/species_',num2str(nspec-1),'/w']) & strfind (variable_list, 'v') 
                                ...
                                time_keep=0;
                                for time=1:ntime
                                    w_time_comp_str=regexprep(info.GroupHierarchy.Groups(i).Groups(ii).Groups(iii).Datasets(time).Name,['/particles/species_',num2str(nspec-1),'/w','/cycle_'], '');
                                    w_time_comp_str=[' ',w_time_comp_str,' '];
                                    w_time_comp=str2double(w_time_comp_str);
                                    if strfind (cycle_list, w_time_comp_str) 
                                      time_keep=time_keep+1;
                                      eval(['w_slab' num2str(nspec-1) '.time' num2str(time_keep) '.PROC' num2str(PROC) '= hdf5read(info.GroupHierarchy.Groups(i).Groups(ii).Groups(iii).Datasets(time));']);
                                      w_time=[w_time;w_time_comp];
                                    end
                                end
                                ntime_keep=time_keep;
...
%%                 
                          ...
                          if strcmp(info.GroupHierarchy.Groups(i).Groups(ii).Groups(iii).Name,['/particles/species_',num2str(nspec-1),'/q']) & strfind (variable_list, 'q') 
                                ...
                                time_keep=0;
                                for time=1:ntime
                                    q_time_comp_str=regexprep(info.GroupHierarchy.Groups(i).Groups(ii).Groups(iii).Datasets(time).Name,['/particles/species_',num2str(nspec-1),'/q','/cycle_'], '');
                                    q_time_comp_str=[' ',q_time_comp_str,' '];
                                    q_time_comp=str2double(q_time_comp_str);
                                    if strfind (cycle_list, q_time_comp_str) 
                                      time_keep=time_keep+1;
                                      eval(['q_slab' num2str(nspec-1) '.time' num2str(time_keep) '.PROC' num2str(PROC) '= hdf5read(info.GroupHierarchy.Groups(i).Groups(ii).Groups(iii).Datasets(time));']);
                                      q_time=[q_time;q_time_comp];
                                    end
                                end
                                ntime_keep=time_keep;
...
%%                 
                          ...
                          if strcmp(info.GroupHierarchy.Groups(i).Groups(ii).Groups(iii).Name,['/particles/species_',num2str(nspec-1),'/ID']) & strfind (variable_list, 'ID') 
                                ...
                                time_keep=0;
                                for time=1:ntime
                                    ID_time_comp_str=regexprep(info.GroupHierarchy.Groups(i).Groups(ii).Groups(iii).Datasets(time).Name,['/particles/species_',num2str(nspec-1),'/ID','/cycle_'], '');
                                    ID_time_comp_str=[' ',ID_time_comp_str,' '];
                                    ID_time_comp=str2double(ID_time_comp_str);
                                    if strfind (cycle_list, ID_time_comp_str) 
                                      time_keep=time_keep+1;
                                      eval(['ID_slab' num2str(nspec-1) '.time' num2str(time_keep) '.PROC' num2str(PROC) '= hdf5read(info.GroupHierarchy.Groups(i).Groups(ii).Groups(iii).Datasets(time));']);
                                      ID_time=[ID_time;ID_time_comp];
                                    end
                                end
                                ntime_keep=time_keep;
